...
`NewExecutorNotTakenBeforeTest` indexes Documents into a `DocumentArray`  using `storage='qdrant'`. Underneath, the `DocumentArray`  uses 
...
## Setup
`NewExecutorNotTakenBeforeTest` requires a running Qdrant server. Make sure a server is up and running and your indexer is configured 
...
Note that if you run a `Qdrant` service locally and try to run the `NewExecutorNotTakenBeforeTest` via `docker`, you 
...
f = Flow().add(
    uses='jinahub+docker://NewExecutorNotTakenBeforeTest',
    uses_with={
        'host': 'localhost',
        'port': 6333,
...
f = Flow().add(uses='jinahub://NewExecutorNotTakenBeforeTest',
    uses_with={
        'host': 'localhost',
        'port': 6333,
        'collection_name': 'collection_name',
...
f = Flow().add(
         uses='jinahub://NewExecutorNotTakenBeforeTest',
...
### Using filtering
To do filtering with the NewExecutorNotTakenBeforeTest you should first define columns and precise the dimension of your embedding space.
For instance :
