...
`NOWQdrantIndexer6` indexes Documents into a `DocumentArray`  using `storage='qdrant'`. Underneath, the `DocumentArray`  uses 
...
## Setup
`NOWQdrantIndexer6` requires a running Qdrant server. Make sure a server is up and running and your indexer is configured 
...
Note that if you run a `Qdrant` service locally and try to run the `NOWQdrantIndexer6` via `docker`, you 
...
f = Flow().add(
    uses='jinahub+docker://NOWQdrantIndexer6',
    uses_with={
        'host': 'localhost',
        'port': 6333,
...
f = Flow().add(uses='jinahub://NOWQdrantIndexer6',
    uses_with={
        'host': 'localhost',
        'port': 6333,
        'collection_name': 'collection_name',
...
f = Flow().add(
         uses='jinahub://NOWQdrantIndexer6',
...
### Using filtering
To do filtering with the NOWQdrantIndexer6 you should first define columns and precise the dimension of your embedding space.
For instance :
