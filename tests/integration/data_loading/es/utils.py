...
def delete_es_index(connector: ElasticsearchConnector, name: str) -> None:
    ...
    """
    connector._es.indices.delete(index=name)
...
def es_insert(
    ...
    success, _ = bulk(connector._es, documents, refresh='wait_for')
    return success
...
def create_es_index(
    ...
    """  # noqa: E501
    if connector._es.indices.exists(index=name):
        connector._es.indices.delete(index=name)
    connector._es.indices.create(index=name, mappings=mapping)
