...
def assert_deployment_queries(
    ...
    if kwargs.secured:
        ...
        response = requests.post(
            ...
            headers={'Authorization': 'key my_key'},
            json=request_body,
        )
        ...
        with pytest.raises(Exception):
            assert_search(
                search_url,
                request_headers={'Authorization': 'key no_key'},
                request_body=request_body,
            )


def get_search_request_kwargs(
    kwargs,
    search_modality,
    dataset=None,
):
...
def assert_search(search_url, request_headers, request_body, expected_status_code=200):
    response = requests.post(
        search_url,
        headers=request_headers,
        json=request_body,
...
def assert_suggest(suggest_url, request_headers, request_body):
