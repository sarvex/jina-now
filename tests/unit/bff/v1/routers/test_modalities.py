...
@pytest.mark.parametrize(
    'input_modality, output_modality, attribute_name_input, attribute_name_output',
    [
        ('image', 'image', 'blob', 'blob'),
        ('image', 'text', 'blob', 'text'),
        ('text', 'image', 'text', 'blob'),
        ('text', 'text', 'text', 'text'),
        ('music', 'music', 'blob', 'blob'),
        ('text', 'video', 'text', 'blob'),
    ],
)
class TestEndpoints:
    def test_index_fails_with_no_flow_running(
        ...
        with pytest.raises(ConnectionError):
            client.post(
                f'/api/v1/{input_modality}-to-{output_modality}/index',
                json={
    ...
    def test_search_fails_with_no_flow_running(
        ...
        with pytest.raises(ConnectionError):
            client.post(
                f'/api/v1/{input_modality}-to-{output_modality}/search',
