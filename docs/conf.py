...
try:
    if 'NOW_VERSION' not in os.environ:
        libinfo_py = path.join(repo_dir, 'now', '__init__.py')
        ...
        __version__ = os.environ['NOW_VERSION']
...
base_url = '/'
html_baseurl = 'https://now.jina.ai'
...
<p>Otherwise, <a href="https://github.com/jina-ai/now/">please create a Github issue</a> and one of our team will respond.</p>
...
linkcheck_ignore = [
    ...
    'http://0.0.0.0:*',
    'pods/encode.yml',
    'https://github.com/jina-ai/now/commit/*',
    '.github/*',
...
ogp_type = 'website'
ogp_site_name = (
    f'NOW {os.environ.get("SPHINX_MULTIVERSION_VERSION", version)} Documentation'
)
...
def setup(app):
    ...
    app.add_config_value(
        name='server_address',
        default=os.getenv('JINA_DOCSBOT_SERVER', 'https://jina-ai-now.docsqa.jina.ai'),
        rebuild='',
    )
