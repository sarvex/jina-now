    ...
    class NOWBaseIndexer(Executor):
        ...
        def get_tags_and_values(self, **kwargs):
            ...
            for key, value_to_count in count_dict.items():
                ...
                tag_to_values[key] = [
                    value
                    for (value, _), _ in zip(
                        sorted_values, range(self.max_values_per_tag)
                    )
                ]
        ...
        def merge_matches_by_score_after_half(
            ...
            for doc, doc_to_add in zip(docs_with_matches, docs_with_matches_to_add):
                ...
                possible_matches = (
                    deepcopy(doc.matches[limit // 2 :]) + doc_to_add.matches
                )
                doc.matches = doc.matches[: limit // 2]
                ids_scores = [
                    (match.id, match.scores[score_name].value)
            ...
            for doc in docs:
                if doc.uri:
                    if doc.text:
                        ...
                        try:
                            if not doc.uri.startswith(
                                f'data:{doc.mime_type}'
                            ) and not any(
                                [
